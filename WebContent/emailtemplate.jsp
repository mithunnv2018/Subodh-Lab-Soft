...
<%JSONArray returnvalue2 =FormUtil.makeemailtemplatelist();%>
...
*/	
    ...
    var data_list = [
				...
				<%JSONObject objjson= returnvalue2.getJSONObject(x);%>
      				{ id: '<%=x%>', col2: '<%=objjson.get("id")%>',col3:'<%=objjson.get("name")%>',col4:'<%=objjson.get("description")%>'}
...
function popGridXMLHTTPJson()
{
	...
	xmlhttp.open("GET","addEmailtemplateMaster.do?methodtocall=getEmailtemplateAll",true);
	xmlhttp.send();
	 ...
	 // alert("hidden: " + document.getElementById("txthiddenid").value + ", edit: " + isEdit);
	  ...
	  if(isEdit==true){
		...
		//alert("values : " + "txtemailconfigname=" + document.getElementById('txtemailconfigname').value + "&txtemailid=" + document.getElementById('txtemailid').value + "&txtemailpass=" + document.getElementById('txtemailpass').value + "&drpemailtype=" + document.getElementById('drpemailtype').value + "&drpMailboxtype=" + document.getElementById('drpMailboxtype').value + "&txtincominghost=" + document.getElementById('txtincominghost').value + "&txtoutgoinghost=" + document.getElementById('txtoutgoinghost').value + "&txtoutgoingportno=" + document.getElementById('txtoutgoingportno').value + "&txtincomingportno=" + document.getElementById('txtincomingportno').value)
	 	...
	 	xmlhttp.open("GET","addEmailtemplateMaster.do?methodtocall=edit&emailtemplateid="+ document.getElementById('txthiddenid').value +"&name=" + document.getElementById('txtsubqueryname').value + "&body=" + escape(dojo.byId('txtdescription').value) ,true);
	 	xmlhttp.send();
		...
		xmlhttp.open("GET","addEmailtemplateMaster.do?methodtocall=add&name=" + document.getElementById('txtsubqueryname').value + "&body=" + escape(dojo.byId('txtdescription').value) ,true);
...
function editdata(){
	...
	if(document.getElementById("txthiddenid").value!=""){
		...
		xmlhttp.open("GET","addEmailtemplateMaster.do?methodtocall=getEmailtemplateDetails&emailtemplateid=" + document.getElementById("txthiddenid").value,true);
		xmlhttp.send();
...
<div id="Accordion1" class="Accordion" tabindex="0">
		  <div class="AccordionPanel">
		   ...
		   <table border=0 width="90%">
			...
			<td>
				<input type="text" id="txtsubqueryname" name="txtsubqueryname" placeholder="Name" size="18">
			</td>
		   	</tr>
		   	<tr>
		   	<td style="font-family: AftasansRegular;font-size: 13px">
