...
<%JSONArray returnvalue2 =FormUtil.makeMainquerylist();%>
...
*/	
    ...
    var data_list = [
				...
				<%JSONObject objjson= returnvalue2.getJSONObject(x);%>
      				{ id: '<%=x%>', col2: '<%=objjson.get("mainquery")%>',col3:'<%=objjson.get("id")%>'}
    ...
    var layout = [[
      ...
      {name: 'Main Query', field: 'col2' ,width: "300px"},
...
function popGridXMLHTTPJson()
{
	...
	xmlhttp.open("GET","addMainqueryMaster.do?methodtocall=getMainqueryAll",true);
	xmlhttp.send();
...
*/
	...
	document.getElementById("txtmainquery").value = "";
...
function savedata(){
	...
	xmlhttp.onreadystatechange=function()
	 ...
	 // alert("hidden: " + document.getElementById("txthiddenid").value + ", edit: " + isEdit);
	  if(document.getElementById("txtmainquery").value==""){
		alert("Please enter main query")
		  }
	  else if(isEdit==true){
		...
		//alert("inside add" + document.getElementById("txthiddenid").value);
	 	...
	 	xmlhttp.open("GET","addMainqueryMaster.do?methodtocall=edit&txtmainquery=" + document.getElementById('txtmainquery').value + "&txtmainqueryid=" + document.getElementById('txthiddenid').value,true);
	 	xmlhttp.send();
		...
		xmlhttp.open("GET","addMainqueryMaster.do?methodtocall=add&txtmainquery=" + document.getElementById('txtmainquery').value,true);
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
			    if(xmlhttp.responseText=="false"){
			    	...
			    	alert("No Such Category Exists");
				    	...
				    	isEdit=true;
				    	//alert(isEdit);
				    	document.getElementById("txtmainquery").value = xmlhttp.responseText; 				    	
					    }
			    
		    }
		  }
		
		xmlhttp.open("GET","addMainqueryMaster.do?methodtocall=getMainqueryName&txtmainqueryid=" + document.getElementById("txthiddenid").value,true);
		xmlhttp.send();
		
	document.getElementById("txtmainquery").value=document.getElementById("txthiddenid").value;
	}else{
...
<div id="Accordion1" class="Accordion" tabindex="0">
		  <div class="AccordionPanel">
		    <div class="AccordionPanelTab"><img src="images/man-user.png" width="20" height="20"/> Add / Modify Main Query</div>
		   ...
		   <table border=0 width="90%">
			...
			<td>
				<input type="text" id="txtmainquery" name="txtmainquery" placeholder="Main Query" size="18">
			</td>
		   	</tr>
		   	<tr>
		   	<td align="right">
			...
			<td>
				<select id="searchcolname" name="searchcolname">
	            	...
	            	<option value=" " selected="selected">Select Column Name</option>
					  <option value="col2">Main Query Category</option>
