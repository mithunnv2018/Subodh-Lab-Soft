...
function callalert(){
	//alert(dijit.Editor.get('value'));
	dojo.byId('field').value = dijit.byId('field').get('value');
	alert(dojo.byId('field').value);
}
...
alert("rowsize: " + <%=returnos.length()%>);
	<%if(returnos!=null){%>
<%for(int xp2=0;xp2<returnos.length();xp2++){%>

	<%JSONObject objjson= returnos.getJSONObject(xp2);%>
			alert("id: " + <%=objjson.get("id")%>); 
	<%}%>
<%}%>
...
<div id="Accordion1" class="Accordion" tabindex="0">
		  <div class="AccordionPanel">
		   ...
		   <body class="soria">
		    <textarea id="field"  name="field" dojoType="dijit.Editor"> 
			 
			</textarea>
		    </body>
		    <input type="button" id="btneditor" name="btneditor"  onclick="callalert();" value="Click fo alert"/> 
		...
		<tr>
		    ...
		    <p>
		<input name="btntry" onclick="ping();" type="button" value="ping" style="background-color:#000; color:#FFF;border-color: #000;border-bottom-color: #000;border-left-color: #000;border-right-color: #000;border-top-color: #000; height: 23px;width: 45px" align="right"/>		    	
