...
<%if(request.getParameter("SUBTAXID")!=null&&!(request.getParameter("SUBTAXID").trim().equals(""))){%>
var isEdit = true;
var parampartyid = <%=request.getParameter("SUBTAXID")%>;
<%}else{%>
var isEdit = false;
<%}%>
...
function validation(){
	loaddoc();
	if(document.getElementById('txtsubtaxheadname').value == ""){
...
function editsave(){
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {
	    	...
	    	//updateGrid();
			document.location = "subtaxhead.do"; 
	...
	var url = 'SubTaxHeadMaster.do?methodtocall=edit&subtaxid=<%=request.getParameter("SUBTAXID")%>&txtsubtaxheadname=' + document.getElementById('txtsubtaxheadname').value + '&drptaxhead=' + document.getElementById('drptaxhead').value + '&txtpercent=' + document.getElementById('txtpercent').value + '&drpyear=' + document.getElementById('drpyear').value + '&drpstatus=' + document.getElementById('drpstatus').value;	//var url='";
...
function savedata(){
	...
	strval = strval + '"}';
		...
		var url = 'SubTaxHeadMaster.do?methodtocall=add&txtsubtaxheadname=' + document.getElementById('txtsubtaxheadname').value + '&drptaxhead=' + document.getElementById('drptaxhead').value + '&txtpercent=' + document.getElementById('txtpercent').value + '&drpyear=' + document.getElementById('drpyear').value + '&drpstatus=' + document.getElementById('drpstatus').value;	//var url='";
				...
				<div class="row-fluid">
					<div class="span12">
						<div class="box box-bordered box-color">
							<div class="box-title">
								<h3><i class="icon-th-list">Sub Tax Head Master</i> </h3>
							</div>
							<div class="box-content nopadding">
							<%JSONArray returnproject=null;%>
							<%if(request.getParameter("SUBTAXID")!=null&&!(request.getParameter("SUBTAXID").trim().equals(""))){%>
							<%JSONArray returnarr=null;%>
							<%returnarr=CommonUtil.getSubTaxHeadEdit(request.getParameter("SUBTAXID"));%>
						...
						<form action="" method="POST" class="form-horizontal form-bordered">
									...
									<div class="control-group">
										<label for="textfield" class="control-label">Sub Tax Head Name </label>
										<div class="controls">
											<!--<input type="text" name="textfield" id="textfield" placeholder="Text input" class="input-xlarge">-->
                                            <input ID="txtsubtaxheadname" value="<%=returnobj.get("name") %>" value= type="text" placeholder="Head Name..."  class='ckeditor span12' rows="5" />
                                            
										</div>
									</div>
									
									<div class="control-group">
										<label for="textfield" class="control-label">Select Tax Head</label>
										<div class="controls">
											<!--<input type="text" name="textfield" id="textfield" placeholder="Text input" class="input-xlarge">-->
                                            <select id="drptaxhead" >
											...
											<%if(returnproject!=null){%>
												<%for(int xp1=0;xp1<returnproject.length();xp1++){%>
													...
													<%JSONObject objjson2= returnproject.getJSONObject(xp1);%>
														<%if(returnobj.get("taxid").equals(objjson2.get("id"))){%>
								...
								<%}else{%>
									...
									<div class="control-group">
										...
										<div class="controls">
											<!--<input type="text" name="textfield" id="textfield" placeholder="Text input" class="input-xlarge">-->
                                            <input ID="txtsubtaxheadname" value="" value= type="text" placeholder="Head Name..."  class='ckeditor span12' rows="5" />
                ...
                <div class="row-fluid">
					<div class="span12">
						<div class="box">
							<div class="box-title">
								<h3>
									<i class="icon-table">Sub Tax Head Master List</i>
					...
					<%returnproject =CommonUtil.getSubTaxheadAll() ;%>
					<%int introw2=0;%>
					<%if(returnproject!=null){%>
							<%for(int xp1=0;xp1<returnproject.length();xp1++){%>
								...
								<tr>
									...
									<td><a href="subtaxhead.do?SUBTAXID=<%=objjson2.get("id")%>">Edit</a>
									</td>
								</tr>
								<%introw2=introw2+1; %>
				  			<%}%>
