...
<%JSONArray returnvalue2 =FormUtil.getPartyAll();%>
...
*/	
    ...
    var data_list = [
				...
				<%JSONObject objjson= returnvalue2.getJSONObject(x);%>
      				{ id: '<%=x%>', col2: '<%=objjson.get("partyid")%>',col3:'<%=objjson.get("fname")%>',col4:'<%=objjson.get("mname")%>',col5:'<%=objjson.get("lname")%>',col6:'<%=objjson.get("cityid")%>'}
    ...
    var layout = [[
      ...
      {name: 'partyid', field: 'col2' ,width: "0px", hidden:true},
      {name: 'First Name', field: 'col3' ,width: "300px"},
      {name: 'Middle Name', field: 'col4' ,width: "200px"},
      {name: 'Last Name', field: 'col5' ,width: "200px"},
      {name: 'City', field: 'col6' ,width: "200px"}
    ]];
...
function updateEmailid(){
	...
	if(document.getElementById("txthiddenid").value!=""){
		...
		<%if(request.getParameter("EMAIL")!=null){%>
			...
			xmlhttp.open("GET","addPartyMaster.do?methodtocall=updateemailid&partyid=" + document.getElementById("txthiddenid").value + "&emailid=<%=request.getParameter("EMAIL")%>",true);
...
<div id="Accordion1" class="Accordion" tabindex="0">
		  <div class="AccordionPanel">
		    <div class="AccordionPanelTab"><img src="images/man-user.png" width="20" height="20"/> Edit Party </div>
		   ...
		   <table border=0 width="90%">
		   	...
		   	<td align="right"> 
		   	<%if(request.getParameter("module").equals("mailticket")){ %>
		   	<input name="btnEdit" onclick="updateEmailid();" type="button" value="Update" style="background-color:#000; color:#FFF;border-color: #000;border-bottom-color: #000;border-left-color: #000;border-right-color: #000;border-top-color: #000; height: 23px;width: 55px" align="right"/></td>
		   	<%}else{ %>
		   	<input name="btnEdit" onclick="editdata();" type="button" value="Edit" style="background-color:#000; color:#FFF;border-color: #000;border-bottom-color: #000;border-left-color: #000;border-right-color: #000;border-top-color: #000; height: 23px;width: 45px" align="right"/></td>
		   	<%} %>
		   	</tr>
