...
<%@page import="com.esaya.TblPartycommuncationDetails" %>
<%@page import="com.esaya.TblPartyMaster" %>
...
<%List<TblPartycommuncationDetails> objpartycom = FormUtil.getPartyComObject(request.getParameter("partyid"));%>

<%if(objpartycom!=null){%>
	introw = <%=objpartycom.size()%>
<%}%>
...
*/	
    ...
    var data_list = [
            ...
            <%if(objpartycom!=null){%>
            <%for(int i=0;i<objpartycom.size();i++){%>
            	<%TblPartycommuncationDetails objpartycom2=objpartycom.get(i);%>
			{ id: '<%=i+1%>', col2: '<%=FormUtil.getPartyComTypeName( objpartycom2.getTblPartycommunicationtypeMaster())%>',col3:'<%=objpartycom2.getPartyComInfostr()%>'},
			<%}%>
			<%}%>
...
function loaddoc(){
	...
	window.setTimeout( loadformdata, 2400 );
}
//alert("beforeload");
	...
	xmlhttp.onreadystatechange=function()
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {		    
			...
			document.getElementById('txtfname').value = arr[0].fname; 
			document.getElementById('txtmname').value = arr[0].mname; 
			document.getElementById('txtlname').value = arr[0].lname; 
			document.getElementById('txtusername').value = arr[0].username; 
			document.getElementById('txtpassword').value = "";
			document.getElementById('drpcountry').value = arr[0].countryid;
			
			document.getElementById('txtaddress').value = arr[0].address; 
			document.getElementById('txtpincode').value = arr[0].pincode; 
			document.getElementById('drppartytype').value = arr[0].partytypeid; 
			
			//document.getElementById('drpState').value = arr[0].stateid;
			
			document.getElementById('drpdepartment').value = arr[0].deptypeid; 
			document.getElementById('drpempcat').value = arr[0].empcatid; 
			document.getElementById('txtemailfooter').value = arr[0].emailfooter;
			document.getElementById('drprole').value = arr[0].roleid;
			
			document.getElementById('drpbrowser').value = arr[0].drpbrowser;
			document.getElementById('drpos').value = arr[0].drpos;
			document.getElementById('drpproject').value = arr[0].drpproject;
			
			//document.getElementById('drpCity').value = arr[0].cityid;
			
			window.setTimeout( popstateonload(arr[0].stateid,arr[0].cityid), 200 );
			//window.setTimeout( popcityonload(arr[0].stateid,arr[0].cityid), 700 ); 			
			isEdit=true;
	    }
    }

	//alert("loadform");
	var url = 'addPartyMaster.do?methodtocall=getPartyDetails&partyid=' + parampartyid;
	//var url='";
...
function popstateonload(stateid,cityid){
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {
		    ...
		    $('#drpState').empty()
		   	...
		   	for(var x=0;x<arr.length;x++){
		   		if(stateid==arr[x].id){  
					$('#drpState').append('<option value="' + arr[x].id + '" selected>' + arr[x].state + '</option>');
		   		}else{
		   			$('#drpState').append('<option value="' + arr[x].id + '" >' + arr[x].state + '</option>');
		   		}
		   	}
		   	popcityonload(stateid,cityid);
	     }
	  }
	//alert("getstateonload");
	xmlhttp.open("GET","addPartyMaster.do?methodtocall=getstate",true);
	xmlhttp.send();

}

function popcityonload(stateid,cityid){
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {
		   ...
		   //	alert("city length: " + arr.length);
		   	for(var x=0;x<arr.length;x++){
		   		if(cityid==arr[x].id){
				$('#drpCity').append('<option value="' + arr[x].id + '" selected>' + arr[x].city + '</option>');
		   		}else{
...
//	  alert(document.getElementById("drpState").value);
	xmlhttp.open("GET","addPartyMaster.do?methodtocall=getcity&stateid=" + stateid,true);
	xmlhttp.send();
